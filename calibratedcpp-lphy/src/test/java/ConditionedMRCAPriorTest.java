...
public class ConditionedMRCAPriorTest {
    ...
    static void generateSamples() {
        ConditionedMRCAPrior prior = new ConditionedMRCAPrior(
                new Value<>("", buildCalibrationSpecs()),
                new Value<>("", P_COV)
        );
    ...
    void testMapBetaNodes() {
        boolean[] actual = mapBetaNodes(buildCalibrationSpecs(), PARENT);
    ...
    void testAllNodesCoverageReasonable() {
        for (int i = 0; i < N; i++) {
            ...
            assertTrue(cov > 0.5,
                    "Node " + (i + 1) + " coverage=" + String.format("%.4f", cov) + " is unreasonably low (< 0.5)");
        }
    }
    ...
    void testRootMeanConsistentWithLogNormal() {
        ...
        assertTrue(pValue > T_TEST_THRESHOLD,
                "Root node t-test rejected lognormal mean. p-value=" + String.format("%.6f", pValue));
    }
    ...
    void testBetaNodeRatiosConsistent() {
        ...
        for (int i : betaNodes) {
            ...
            assertTrue(pValue > T_TEST_THRESHOLD,
                    "Node " + (i + 1) + ": beta ratio t-test rejected. p-value=" + String.format("%.6f", pValue));
        }
    }
    ...
    void testPerNodeCoverageMatchesRScript() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            assertTrue(cov >= minCoverage,
                    "Node " + (i + 1) + ": coverage=" + String.format("%.4f", cov) + " < " + minCoverage);
        }
    }
    ...
    private static String[][] buildDummyTaxa() {
        return new String[][] {
                {"t1","t2","t3","t4","t5","t6","t7","t8","t9","t10", "t11","t12", "t13","t14"}, // node 0: root
                {"t1","t2","t3","t4","t5","t6","t7","t8","t9","t10"},                            // node 1: child of 0
                {"t11","t12", "t13","t14"},                                                       // node 2: child of 0
                {"t11","t12"},                                                                    // node 3: child of 2, beta
                {"t1","t2"},                                                                      // node 4: child of 1, beta
                {"t3","t4","t5", "t6","t7","t8","t9","t10"},                                     // node 5: child of 1
                {"t3","t4"},                                                                      // node 6: child of 5
                {"t5", "t6", "t7", "t8", "t9", "t10"},                                           // node 7: child of 5
                {"t5", "t6", "t7","t8","t9"},                                                    // node 8: child of 7, beta
                {"t8","t9"},                                                                      // node 9: child of 8, beta
        };
    }
}
